...
import { ftg256, soic8w208, qfn16, osc3225, inductor, type LandPattern } from "./footprints"
...
const c=(name:string,value:string,x:number,y:number,a:string,b="GND",footprint="0603",layer:"top"|"bottom"="top",rotation=0)=>passive("capacitor",name,value,x,y,a,b,footprint,layer,undefined,rotation)
...
j("J1","5V INPUT",-70,35,["VIN5","GND"],90)
...
pdn.forEach((net,i)=>c(`C${100+i}`,"100nF",-7+(i%6)*3,12-Math.floor(i/6)*3,net,"GND","0201","bottom",45))
...
j("J2","JTAG: VREF GND TCK TDO TDI TMS",8,-40,["V3V3","GND","JTAG_TCK","JTAG_TDO","JTAG_TDI","JTAG_TMS"],0)
r("R103","10k",4,-30,"JTAG_TMS","V3V3")
r("R104","10k",8,-30,"JTAG_TDI","V3V3")
r("R105","10k",12,-30,"JTAG_TCK","GND")
j("J3","PROGRAM / GND",23,-40,["PROGRAM_B","GND"],0)  // 23, not 27: clears U4's courtyard
j("J4","RESET / GND",-10,-40,["reset_n","GND"],0)
...
j("J5","STEREO LINE OUT L/G/R",71,8,["LINE_L","GND","LINE_R"],270)
...
add("U4","MCP3208-CI/SL",35,-39,"soic16_p1.27mm",{
  ...
  pin6:"GND",pin7:"GND",pin8:"GND",pin9:"GND",pin10:"adc_cs_n",pin11:"adc_mosi",
...
for(const [i,net] of ["POT_PITCH","POT_DECAY","POT_TIMBRE"].entries()){
  j(`J${6+i}`,`${net}: REF/WIPER/GND`,-36+i*18,40,["VREF25",`${net}_RAW`,"GND"],0)
  r(`R${131+i}`,"1k",-36+i*18,34,`${net}_RAW`,net)
  c(`C${174+i}`,"10nF",potFilter[i][0],potFilter[i][1],net)
}
j("J9","ENCODER A/GND/B/SW/GND",21,40,["ENC_A_RAW","GND","ENC_B_RAW","ENC_SW_RAW","GND"],0)
...
j("J10","MODE: 3V3/CV_SEL/GND",45,40,["V3V3","cv_select","GND"],0)
...
j("J11","STATUS: GND/VOICE0..3",64,40,["GND","LED0_A","LED1_A","LED2_A","LED3_A"],0)
...
for(const [i,net] of ["PITCH","MOD"].entries()){
  const y=-18-i*13
  j(`J${12+i}`,`${net} CV / GND`,72,y,[`${net}_IN`,"GND"],270)
...
j("J14","GATE / GND",71,-41,["GATE_IN","GND"],270)
...
j("J15","MIDI DIN4 / DIN5",-72,-40,["MIDI4","MIDI5"],90)
